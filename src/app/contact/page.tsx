    ...
    <div className="bg-[#FFF8F0] min-h-screen">
      ...
      <section className="bg-[#3D2B1F] text-white py-16">
        <div className="max-w-3xl mx-auto px-4 text-center">
          <Mail className="w-14 h-14 text-[#F4A460] mx-auto mb-4" />
      ...
      <section className="max-w-2xl mx-auto px-4 py-16">
        <form onSubmit={handleSubmit} className="bg-white rounded-2xl border border-[#F5E6D3] p-8 shadow-sm space-y-5">
          ...
          <div>
            <label className="block text-sm font-medium text-[#3D2B1F] mb-1">Téléphone (optionnel)</label>
            <input
              type="tel"
              value={form.phone}
          ...
          <div>
            <label className="block text-sm font-medium text-[#3D2B1F] mb-1">Message *</label>
            <textarea
              value={form.message}
              onChange={(e) => setForm({ ...form, message: e.target.value })}
              required
              rows={5}
              className="w-full border border-[#D2B48C] rounded-xl px-3 py-2 text-sm focus:outline-none focus:border-[#8B4513] resize-none"
            />
          </div>
          ...
          <p className="text-xs text-center text-[#A0856B]">Réponse sous 48h ouvrées</p>
        </form>
      </section>
    </div>
  );
